...
from torch_geometric.nn import NNConv, Set2Set, MessagePassing, aggr, GATConv
...
model_name = "b128_e300_adam_mse_gatconv_poolMaxAggr_2fc_wandb"
...
def train_one_epoch(device, model, train_loader, epoch, optimizer): 
    # Train step
    model.train()
    total_loss = 0
...
def evaluate_one_epoch(device, model, val_loader, epoch): 
    # Validation step
    model.eval()
    total_val_loss = 0
...
def test_step(device, model, test_loader): 
    # Test step
    model.eval()
    total_test_loss = 0
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            test_loss_output = test_loss(output, data.y)
            total_test_loss += test_loss_output * data.num_graphs
...
class GCN(torch.nn.Module):
    def __init__(self, in_channels, hidden_dim_1, hidden_dim_2, hidden_dim_3, out_channels, num_heads, dropout):
        super().__init__()
        self.conv1 = GATConv(in_channels=in_channels, out_channels=hidden_dim_1, heads=num_heads, dropout=dropout)
        self.conv2 = GATConv(in_channels=hidden_dim_1*num_heads, out_channels=hidden_dim_2,  dropout=dropout)
...
def build_model(in_channels, hidden_dim_1, hidden_dim_2, hidden_dim_3, out_channels, num_heads, dropout):
    # Initialize the model, optimizer, and loss function
    model = GCN(in_channels, hidden_dim_1, hidden_dim_2, hidden_dim_3, out_channels, num_heads, dropout)
    print(model)
    logging.info(model)
    return model
...
def main(config):
    ...
    # Build gnn model
    model = build_model(in_channels, hidden_dim_1, hidden_dim_2, hidden_dim_3, out_channels,num_heads, dropout)
    model.to(device)
    # Build optimizer
    ...
    for epoch in range(epochs):
        train_loss = train_one_epoch(device, model, train_loader, epoch, optimizer)
        #loss_values.append(train_loss)
        val_loss = evaluate_one_epoch(device, model, val_loader, epoch)
    ...
    test_loss = test_step(device, model, test_loader)
    wandb.log({
            'test_loss': test_loss
        })
    # optional: save model at the end
    model.to_onnx()
    wandb.save("model_e300.onnx")
